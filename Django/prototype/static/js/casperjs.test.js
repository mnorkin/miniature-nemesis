...
casper.then(function() {
    ...
    this.test.assert(
        document.querySelectorAll('#popup .list a').length === 0,
        'Popup content is empty'
    );
...
casper.then(function() {
    ...
    this.test.assertEval(function(){
        return document.querySelectorAll('.grid #target-price-list .entry').length >= 3;
    }, 'Title target price grid blocks 3 or more');

    this.test.assertEval(function(){
        return document.querySelectorAll('.grid #target-price-list .bar svg').length >= 9;
    }, 'Title target prices grid blocks svg 9 or more');

    // change to list view
    this.click('a.toggle');
    this.test.assertEval(function(){
        return document.querySelectorAll('.list #target-price-list .entry').length >= 3;
    }, 'Title target price list blocks 3 or more');

    this.test.assertEval(function() {
        return document.querySelectorAll('.list #target-price-list .bar svg').length >= 9;
...
casper.thenOpen(test_host + 'ticker/aapl/', function(){
    ...
    this.test.assertEval(function(){
        return document.querySelectorAll('#bank li.active').length === 1;
    }, 'Ticker page have active/best bank');

    this.test.assertEval(function(){
        return document.querySelectorAll('.analyse_menu div a.active').length === 1;
    }, 'Ticker page have active feature');

    this.test.assertEval(function(){
        return document.querySelectorAll('#chart svg').length === 1;
    ...
    casper.waitFor(
        ...
        function(){
            this.test.assert(false, 'Ticker page loaded aggressiveness graph');
        }
    );

    this.test.assertEval(function(){
        return document.querySelectorAll('.analyse_menu div.can_compare').length === 1;
    ...
    casper.waitFor(
        ...
        function(){
            this.test.assert(false, 'Ticker page loaded target pricess list');
        }
    );
...
casper.thenOpen(test_host + 'analytic/citigroup/', function(){
    ...
    casper.waitFor(
        ...
        function(){
            this.test.assert(false, 'Analytic page loaded target pricess list');
        }
    );
